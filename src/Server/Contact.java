...
public class Contact{
    ...
    public static boolean userExists(String name, String ip, int port){
        if(contacts.stream().anyMatch(c -> c.getUserName().equals(name))) return true;
        if(contacts.stream().anyMatch(c -> c.getIp().toString().equals("/"+ip)
                && contacts.stream().anyMatch(c2 -> c2.getPort() == port))) return true;
        return false;
    }

    public static boolean userExists(String name, InetAddress ip, int port){
        if(contacts.stream().anyMatch(c -> c.getUserName().equals(name))) return true;
        if(contacts.stream().anyMatch(c -> c.getIp().equals(ip)
                && contacts.stream().anyMatch(c2 -> c2.getPort() == port))) return true;
        return false;
    }

    public static Contact getContactByName(String name) throws NoSuchElementException{
        return contacts.stream().filter(c -> c.getUserName().equals(name))
                .findFirst()
    ...
    public void updateContact(InetAddress ip, int port){
        this.ip = ip;
        this.port = port;
    }
