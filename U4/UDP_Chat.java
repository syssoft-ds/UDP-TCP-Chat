...
public class UDP_Chat {
    ...
    private static final Map<String, Boolean> statusClients = new HashMap<>();
    ...
    private static Boolean isConnected(String client) {
        ...
        try (DatagramSocket s = new DatagramSocket()) {
            ...
            InetAddress ip = InetAddress.getByName(clientInfo.ip);
            byte[] buffer = message.getBytes(StandardCharsets.UTF_8);
            DatagramPacket p = new DatagramPacket(buffer, buffer.length, ip, clientInfo.port);
            s.send(p);
            ...
            return responseMessage.equalsIgnoreCase("yes");
        } catch (Exception e) {
            return false;
        }
    }
